...
export async function POST(req: NextRequest) {
  try {
    ...
    if (user.role !== "ADMIN") {
      return NextResponse.json({ error: "Access denied. This endpoint is for administrators only." }, { status: 403 })
    }
    ...
    const { data, error } = await resend.emails.send({
      ...
      to: [email],
      subject: 'Your Magic Link - AI Booking Agent Admin',
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
          <h2 style="color: #dc2626;">🔐 Magic Link for AI Booking Agent</h2>
          <p>Hi ${user.name || email},</p>
          <p>You requested a magic link to access your admin account. Click the button below to sign in:</p>
      ...
      `,
      text: `Magic Link for AI Booking Agent Admin\n\nHi ${user.name || email},\n\nYou requested a magic link to access your admin account. Click the link below to sign in:\n\n${magicLink}\n\nThis link will expire in 15 minutes and can only be used once.\n\nIf you didn't request this link, please ignore this email.\n\nNeed help? Contact us at support@franc-dev.space`,
    })
